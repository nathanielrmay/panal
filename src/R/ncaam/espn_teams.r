...
# 1. FETCH DATA
# ==========================================
print(paste("Fetching ESPN MBB Team metadata for season:", SEASON))

df <- hoopR::espn_mbb_teams(year = SEASON)

# Convert to standard data frame to avoid signature errors
df <- as.data.frame(df)

print(paste("Fetched", nrow(df), "teams from ESPN."))
...
tryCatch({
  ...
  print("Success: ESPN Team data written to database.")
}, error = function(e) {
