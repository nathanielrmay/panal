...
# 1. FETCH DATA
# ==========================================
print(paste("Fetching ESPN MBB Rankings for season:", SEASON))

df <- hoopR::espn_mbb_rankings()

# Convert to standard data frame
df <- as.data.frame(df)

print(paste("Fetched", nrow(df), "ranking entries."))
...
tryCatch({
  ...
  print("Success: ESPN Rankings data written to database.")
}, error = function(e) {
