...
When completing tasks or building features, if a clearly superior standard or library exists, but it requires installing additional OS-level software or modules (like `php-curl`, apt packages, or R dependencies), you MUST inform the user and ask for their preference before falling back to an inferior solution. The user is willing to install necessary packages if it results in better code.
